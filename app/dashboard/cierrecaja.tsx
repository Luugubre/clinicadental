...
import { 
  Wallet, CashIcon, CreditCard, ArrowRightLeft, 
...
export default function CierreCaja() {
  ...
  if (loading) return <div className="p-8 text-center animate-pulse text-slate-400 font-bold">Calculando caja...</div>

  return (
    <div className="bg-white rounded-[3rem] p-8 shadow-xl border border-slate-100 overflow-hidden relative">
      {/* Decoración de fondo */}
      <div className="absolute -top-10 -right-10 text-slate-50 opacity-[0.03]">
        <DollarSign size={200} />
      </div>
      ...
      <div className="relative z-10">
        <header className="flex justify-between items-center mb-8">
          <div>
            <h3 className="text-2xl font-black text-slate-900 tracking-tighter">Cierre de Caja</h3>
            <p className="text-slate-400 text-xs font-bold uppercase tracking-widest mt-1">Resumen del día</p>
          </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-slate-900 rounded-[2rem] p-6 text-white shadow-2xl shadow-slate-200">
            <p className="text-[10px] font-black uppercase tracking-[0.2em] opacity-50 mb-1">Recaudación Total</p>
            <p className="text-4xl font-black tracking-tighter">${stats.total.toLocaleString('es-CL')}</p>
          ...
          <div className="grid grid-cols-1 gap-3">
            <MetodoRow 
              icon={<Wallet className="text-orange-500" size={18} />} 
              label="Efectivo" 
        ...
        <button 
          ...
          className="w-full mt-8 py-4 bg-slate-50 text-slate-400 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-slate-100 hover:text-slate-600 transition-all"
        >
          Actualizar Cierre
        </button>
      </div>
