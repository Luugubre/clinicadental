  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] p-8 pb-20">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="bg-white p-10 rounded-[3rem] shadow-sm border border-slate-100 flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="flex items-center gap-6">
            ...
            <div>
              <h1 className="text-3xl font-black text-slate-800 uppercase italic leading-none">Inventario</h1>
              <p className="text-slate-400 text-[10px] font-bold uppercase tracking-[0.3em] mt-3">Bodega Central - Centro Médico Dignidad</p>
            </div>
          </div>
        ...
        <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
          <div className="relative w-full md:w-96">
            ...
            <input 
              ...
              className="w-full bg-white p-4 pl-12 rounded-2xl text-xs font-bold border-none outline-none shadow-sm"
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50/50">
                <th className="px-10 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">Nombre Producto</th>
                ...
                <th className="px-10 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">Precio Prom.</th>
                <th className="px-10 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">Precio Venta</th>
            ...
            <tbody className="divide-y divide-slate-50">
              {productosFiltrados.map((p) => (
                <tr key={p.id} className="hover:bg-slate-50/50 transition-colors">
                  <td className="px-10 py-6 font-black text-slate-700 uppercase text-xs italic">{p.nombre}</td>
                  ...
                  <td className="px-10 py-6 text-xs font-bold text-slate-400">${p.precio_promedio_compra.toLocaleString()}</td>
                  <td className="px-10 py-6 text-xs font-bold text-blue-600">${p.precio_venta.toLocaleString()}</td>
      ...
      <AnimatePresence>
        {modalCrear && (
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[500] flex items-center justify-center p-4">
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} className="bg-white w-full max-w-lg rounded-[3.5rem] shadow-2xl overflow-hidden">
              <div className="p-8 border-b border-slate-50 flex justify-between items-center bg-slate-50/50">
                <h2 className="text-xl font-black text-slate-800 uppercase italic">Nuevo Insumo</h2>
                <button onClick={() => setModalCrear(false)} className="text-slate-300 hover:text-red-500 transition-colors"><X size={24}/></button>
              </div>
              <div className="p-10 space-y-6">
                <Input label="Nombre del Producto" value={nuevoProd.nombre} onChange={(v) => setNuevoProd({...nuevoProd, nombre: v})} />
                <div className="grid grid-cols-2 gap-4">
                  <Input label="Stock Inicial" type="number" value={nuevoProd.stock_actual} onChange={(v) => setNuevoProd({...nuevoProd, stock_actual: parseInt(v)})} />
                  <Input label="Stock Seguridad" type="number" value={nuevoProd.stock_seguridad} onChange={(v) => setNuevoProd({...nuevoProd, stock_seguridad: parseInt(v)})} />
                  <Input label="Precio Compra ($)" type="number" value={nuevoProd.precio_promedio_compra} onChange={(v) => setNuevoProd({...nuevoProd, precio_promedio_compra: parseInt(v)})} />
                  <Input label="Precio Venta ($)" type="number" value={nuevoProd.precio_venta} onChange={(v) => setNuevoProd({...nuevoProd, precio_venta: parseInt(v)})} />
                </div>
      ...
      <AnimatePresence>
        {modalEgreso && (
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[500] flex items-center justify-center p-4">
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} className="bg-white w-full max-w-md rounded-[3rem] shadow-2xl p-10">
              <h2 className="text-xl font-black text-slate-800 uppercase italic mb-8 flex items-center gap-2"><ArrowDownLeft className="text-red-500" /> Salida de Insumo</h2>
              <div className="space-y-6">
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2 italic">Producto</label>
                  <select 
                    className="w-full bg-slate-50 p-5 rounded-2xl text-xs font-bold outline-none border-none shadow-inner"
                ...
                <Input label="Cantidad a Retirar" type="number" value={egreso.cantidad} onChange={(v) => setEgreso({...egreso, cantidad: parseInt(v)})} />
...
function Input({ label, value, onChange, type = "text" }: any) {
  return (
    <div className="space-y-2">
      <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2 italic">{label}</label>
      <input 
