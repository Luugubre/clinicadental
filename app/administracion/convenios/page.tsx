    ...
    <div className="min-h-screen bg-[#F8FAFC] p-8 pb-20">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="bg-white p-10 rounded-[3rem] shadow-sm border border-slate-100 flex justify-between items-center">
          <div>
            <h1 className="text-3xl font-black text-slate-800 uppercase italic leading-none flex items-center gap-4">
              <Building2 className="text-blue-600" size={32} /> Convenios
            </h1>
            <p className="text-slate-400 text-[10px] font-bold uppercase tracking-[0.3em] mt-3">Gestión de alianzas y beneficios</p>
          </div>
          <button 
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {convenios.map(conv => (
            <motion.div 
              ...
              className="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm hover:border-blue-400 transition-all group cursor-pointer"
            >
              ...
              <h3 className="text-lg font-black text-slate-800 uppercase leading-tight">{conv.nombre_empresa}</h3>
              <p className="text-blue-500 text-[10px] font-bold uppercase tracking-widest mt-1">{conv.nombre_convenio}</p>
              
              <div className="mt-4 pt-4 border-t border-slate-50 space-y-2">
                <p className="text-[10px] text-slate-400 flex items-center gap-2 font-bold uppercase tracking-tighter">
        ...
        {modalAbierto && (
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[500] flex items-center justify-center p-4">
            <motion.div 
              ...
              className="bg-white w-full max-w-5xl max-h-[90vh] rounded-[3.5rem] overflow-hidden shadow-2xl flex flex-col"
            >
              ...
              <div className="flex-1 overflow-y-auto p-10 space-y-12 custom-scrollbar">
                ...
                <section className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <Input label="Nombre Empresa" value={form.nombre_empresa} onChange={(v) => setForm({...form, nombre_empresa: v})} />
                    <Input label="Nombre Convenio" value={form.nombre_convenio} onChange={(v) => setForm({...form, nombre_convenio: v})} />
                    <Input label="RUT" value={form.rut} onChange={(v) => setForm({...form, rut: v})} />
                    <Input label="E-Mail" value={form.email} icon={<Mail size={12}/>} onChange={(v) => setForm({...form, email: v})} />
                    <Input label="Teléfono 1" value={form.telefono_1} onChange={(v) => setForm({...form, telefono_1: v})} />
                    <Input label="Teléfono 2" value={form.telefono_2} onChange={(v) => setForm({...form, telefono_2: v})} />
                    <Input label="Ciudad" value={form.ciudad} onChange={(v) => setForm({...form, ciudad: v})} />
                    <Input label="Comuna" value={form.comuna} onChange={(v) => setForm({...form, comuna: v})} />
                    <Input label="Dirección" value={form.direccion} icon={<MapPin size={12}/>} onChange={(v) => setForm({...form, direccion: v})} />
                    <Input label="Persona de Contacto" value={form.persona_contacto} onChange={(v) => setForm({...form, persona_contacto: v})} />
                ...
                <section className="space-y-6 bg-slate-50 p-10 rounded-[3rem] border border-slate-100 shadow-inner">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    <div className="space-y-3">
                      <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2">Arancel de Precios</label>
                      <select 
                        className="w-full bg-white p-5 rounded-2xl text-xs font-bold border-none outline-none shadow-sm"
                    ...
                    <div className="space-y-3">
                      <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2">Descuento (%)</label>
                      <select 
                        className="w-full bg-white p-5 rounded-2xl text-xs font-bold border-none outline-none shadow-sm"
                    ...
                    <div className="space-y-3">
                      <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2">Estado / Visibilidad</label>
                      <div className="grid grid-cols-2 gap-2">
                        <select 
                          className="bg-white p-4 rounded-xl text-[10px] font-black uppercase outline-none shadow-sm"
...
function Input({ label, value, onChange, icon }: any) {
  return (
    <div className="space-y-2">
      <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2 italic flex items-center gap-2">
        {icon} {label}
      </label>
      <input 
        type="text" value={value || ''} onChange={(e) => onChange(e.target.value)}
        className="w-full bg-slate-50 p-4 rounded-2xl text-xs font-bold border-none outline-none focus:ring-2 ring-blue-500/20 focus:bg-white transition-all shadow-inner"
      />
    </div>
  )
}
