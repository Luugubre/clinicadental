...
import { 
  ...
  Activity, Pill, Heart, CheckCircle2, ChevronDown, X 
} from 'lucide-react'
...
export default function AntecedentesPage() {
  ...
  useEffect(() => { if (id) fetchAntecedentes() }, [id])
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 pb-20 p-4">
      ...
      <div className="bg-white p-10 rounded-[3rem] shadow-sm border border-slate-100 flex items-center justify-between">
         <div>
            <h3 className="text-3xl font-black tracking-tighter text-slate-800 uppercase italic leading-none">Anamnesis Médica</h3>
            <p className="text-slate-400 text-[10px] font-bold uppercase tracking-[0.3em] mt-2">Ficha de riesgos sistémicos</p>
         </div>
...
function SeccionDesplegable({ titulo, categoria, icon, color, opciones, seleccionados, onToggle, onAddCustom }: any) {
  ...
  return (
    <div className="bg-white p-8 rounded-[3.5rem] border border-slate-100 shadow-sm relative h-full">
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-4">
          <div className={`p-4 rounded-[1.5rem] border ${colorMap[color]}`}>{icon}</div>
          <h4 className="font-black text-slate-800 uppercase italic text-lg tracking-tighter">{titulo}</h4>
        </div>
      ...
      {/* CUADRO DESPLEGABLE DE OPCIONES */}
      <AnimatePresence>
        {abierto && (
          <motion.div 
            initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}
            className="absolute top-24 left-8 right-8 bg-slate-900 rounded-[2.5rem] p-8 z-50 shadow-2xl border border-slate-700"
          >
            <p className="text-[9px] font-black text-slate-500 uppercase tracking-[0.3em] mb-6">Selección Rápida</p>
            <div className="flex flex-wrap gap-2 mb-8">
              {opciones.map((opt: string) => {
                const isActive = seleccionados.some((i: any) => i.contenido === opt)
                return (
                  <button 
                    key={opt}
            ...
            <div className="border-t border-slate-800 pt-6">
               <p className="text-[9px] font-black text-slate-500 uppercase tracking-[0.3em] mb-4">Otro / Personalizado</p>
               <div className="flex gap-2">
                  <input 
                    ...
                    onKeyDown={(e) => { if(e.key === 'Enter') { onAddCustom(categoria, inputManual); setInputManual(''); } }}
                  />
                  <button 
                    onClick={() => { onAddCustom(categoria, inputManual); setInputManual(''); }}
      ...
      {/* LISTA DE ITEMS SELECCIONADOS (LO QUE EL PACIENTE TIENE) */}
      <div className="space-y-3">
        {seleccionados.length === 0 ? (
          ...
          seleccionados.map((p: any) => (
            <div key={p.id} className="flex justify-between items-center bg-slate-50 px-6 py-4 rounded-2xl group border border-transparent hover:border-slate-200 transition-all">
              <div className="flex items-center gap-3">
                <CheckCircle2 size={14} className="text-emerald-500" />
                <span className="text-xs font-black text-slate-700 uppercase tracking-tight">{p.contenido}</span>
              </div>
              <button 
