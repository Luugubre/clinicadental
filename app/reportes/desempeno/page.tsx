...
export default function PanelDesempenoPage() {
  ...
  async function fetchMetrics() {
    ...
    try {
      ...
      const abonosPorEspecialista = pagosData?.reduce((acc: any, curr: any) => {
        const nombre = curr.perfiles?.nombre_completo || 'Clínica / General'
      ...
      const atencionesConvenio = atencionesData?.reduce((acc: any, curr: any) => {
        const convenio = curr.pacientes?.prevision || 'Particular'
      ...
      const ventasCajaMesActual = cajasData?.reduce((acc, caja) => {
        const neto = caja.estado === 'cerrada' 
          ? (Number(caja.monto_cierre || 0) - Number(caja.monto_apertura || 0))
          : (caja.pagos?.reduce((sum: number, p: any) => sum + Number(p.monto), 0) || 0)
        return acc + neto
      }, 0) || 0
      ...
      for (let i = 5; i >= 0; i--) {
        ...
        const totalVentasM = historialCajas?.filter(c => {
          const cDate = new Date(c.fecha_apertura); return cDate.getMonth() === mIdx && cDate.getFullYear() === yVal
        }).reduce((acc, c) => acc + (Number(c.monto_cierre || 0) - Number(c.monto_apertura || 0)), 0) || 0
        const totalRecM = historialPagos?.filter(p => {
      ...
      setData({
        agenda: {
          ...
          anuladas: citasData?.filter(c => c.estado === 'anulada').length || 0,
          presupuestos: totalPresupuestos || 0,
          ocupacion: citasData?.length ? Math.round((citasData.filter(c => c.estado === 'finalizada').length / citasData.length) * 100) : 0,
          espera: "17.9"
        },
        financiero: { ventas: ventasCajaMesActual, recaudacion: pagosData?.reduce((acc, p) => acc + Number(p.monto), 0) || 0 },
        charts: {
          ventasProf: chartVentasProf || [],
          pieConvenio: chartPieConvenio || [],
          history: chartHistory || [],
          convenios: Object.entries(pagosData?.reduce((acc: any, curr: any) => {
            const conv = curr.convenio || 'Particular'; acc[conv] = (acc[conv] || 0) + Number(curr.monto); return acc
  ...
  if (loading || !data) return (
    <div className="h-screen flex flex-col items-center justify-center gap-4 bg-slate-50">
      ...
      <p className="font-black text-[10px] uppercase tracking-widest text-slate-400 italic">Sincronizando Dignidad SpA...</p>
    </div>
  )
    ...
    <main className="min-h-screen bg-[#F8FAFC] p-8 font-sans text-slate-900">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center bg-white p-8 rounded-[3rem] shadow-sm border border-slate-100 gap-6">
          <div>
            <h1 className="text-3xl font-black uppercase italic tracking-tighter text-slate-800 leading-none">Panel de Desempeño</h1>
            <p className="text-[10px] font-bold text-blue-600 uppercase tracking-[0.3em] mt-2">CENTRO MEDICO Y DENTAL DIGNIDAD SPA</p>
          </div>
          <div className="flex items-center gap-3">
            <select value={mes} onChange={(e) => setMes(Number(e.target.value))} className="bg-slate-50 border-none rounded-2xl py-3 px-6 text-xs font-black uppercase outline-none focus:ring-2 ring-blue-500/20 cursor-pointer shadow-sm">
            ...
            <select value={anio} onChange={(e) => setAnio(Number(e.target.value))} className="bg-slate-50 border-none rounded-2xl py-3 px-6 text-xs font-black outline-none focus:ring-2 ring-blue-500/20 cursor-pointer shadow-sm">
              {[2024, 2025, 2026].map(a => <option key={a} value={a}>{a}</option>)}
            </select>
            <button onClick={fetchMetrics} className="bg-slate-900 text-white p-3 rounded-2xl shadow-lg hover:bg-blue-600 transition-all active:scale-95"><Filter size={18} /></button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
          ...
          <MetricCard title="Atendidos" value={`${data.agenda.ocupacion}%`} icon={<CheckCircle2 size={20}/>} color="blue" />
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div className="bg-white p-10 rounded-[3.5rem] shadow-sm border border-slate-100">
            <div className="flex justify-between items-start mb-8">
              <div>
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none">Producción Clínica (Ventas)</p>
                <h2 className="text-4xl font-black text-slate-800 mt-3">${Number(data.financiero.ventas).toLocaleString('es-CL')}</h2>
                <span className="text-blue-500 text-[10px] font-bold flex items-center gap-1 mt-2"><Briefcase size={12}/> Flujo neto de cajas</span>
              </div>
              <div className="bg-blue-50 p-4 rounded-3xl text-blue-600 shadow-inner"><TrendingUp size={24}/></div>
            </div>
          ...
          <div className="bg-white p-10 rounded-[3.5rem] shadow-sm border border-slate-100">
            <div className="flex justify-between items-start mb-8">
              <div>
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none">Recaudación (Ingresos)</p>
                <h2 className="text-4xl font-black text-emerald-600 mt-3">${Number(data.financiero.recaudacion).toLocaleString('es-CL')}</h2>
              ...
              <div className="bg-emerald-50 p-4 rounded-3xl text-emerald-600 shadow-inner"><DollarSign size={24}/></div>
            </div>
            <div className="h-[280px] w-full">
              <ResponsiveContainer width="100%" height="100%">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div className="bg-white p-10 rounded-[3.5rem] shadow-sm border border-slate-100">
            ...
            <div className="h-[400px]">
              {(data?.charts?.ventasProf?.length ?? 0) > 0 ? (
                <ResponsiveContainer width="100%" height="100%">
                ...
                <div className="h-full flex items-center justify-center text-slate-300 font-black uppercase text-[10px]">Sin abonos registrados</div>
              )}
            </div>
          </div>
          ...
          <div className="bg-white p-10 rounded-[3.5rem] shadow-sm border border-slate-100">
            ...
            <div className="h-[400px]">
              {(data?.charts?.pieConvenio?.length ?? 0) > 0 ? (
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie
                ...
                <div className="h-full flex items-center justify-center text-slate-300 font-black uppercase text-[10px]">Sin flujo de pacientes</div>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 pb-10">
          <div className="bg-slate-900 p-10 rounded-[3.5rem] text-white flex flex-col justify-between shadow-2xl relative overflow-hidden">
            ...
            <div className="space-y-6 relative z-10">
              <div className="flex items-center gap-4">
                <div className="bg-blue-600 p-3 rounded-2xl shadow-lg shadow-blue-500/20"><Clock size={24} /></div>
                <p className="text-[10px] font-black uppercase tracking-[0.3em]">Eficiencia Operativa</p>
              </div>
              <h4 className="text-5xl font-black italic tracking-tighter">{data.agenda.espera} min</h4>
              <p className="text-[11px] text-slate-400 uppercase font-bold tracking-widest italic">Tiempo promedio real</p>
            </div>
          ...
          <div className="bg-white p-10 rounded-[3.5rem] border border-slate-100 shadow-sm flex flex-col justify-center items-center text-center space-y-4">
             <div className="p-4 bg-blue-50 text-blue-600 rounded-full"><Stethoscope size={32} /></div>
             <h3 className="text-lg font-black uppercase italic italic">Distribución de Flujo</h3>
             <p className="text-xs text-slate-500 font-medium max-w-xs">Convenio con mayor flujo: <strong>{data?.charts?.pieConvenio?.[0]?.name || 'Ninguno'}</strong>.</p>
...
function MetricCard({ title, value, icon, color, subtitle }: any) {
  ...
  return (
    <div className="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm flex flex-col items-center text-center group hover:scale-105 transition-all cursor-default">
      <div className={`${colors[color]} p-4 rounded-2xl mb-4 group-hover:rotate-12 transition-transform shadow-sm`}>{icon}</div>
      <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-2 leading-none">{title}</p>
      <h3 className="text-3xl font-black text-slate-800 leading-none">{value}</h3>
      {subtitle && <p className="text-[8px] font-bold text-slate-400 mt-2 uppercase tracking-tighter">{subtitle}</p>}
    </div>
  )
}